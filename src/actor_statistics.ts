...
class ActorStatistics {
    ...
    saveError(urlString: string, error: string, payload: string | undefined) {
        const url = new URL(urlString);
        const key = `${url.pathname}?${url.searchParams.toString()}`;
        const formattedPayload = payload ? `, Payload: ${payload}` : '';

        if (!this.errors[key]) this.errors[key] = [];
        this.errors[key].push(error.toString() + formattedPayload);
    }
